...
istream& operator>>(istream& input, stack_sorted& stk) {
    int initial_length;
	int i, n;
	float elem;
	initial_length = stk.length;
	cout<<"Input stack_sorted name: ";
	input>>stk.name;          
	cout<<"Input number of elements: ";
  	input>>n;
  	for(i = initial_length + 1; i <= initial_length + n; i++) {
...
ostream& operator<<(ostream& output, stack_sorted& stk) {
        
  output << "Stack_sorted name is: " << stk.name << endl;
	int i;
  stackElement* sEP;
  i = stk.length;
  sEP = stk.header;
...
void stack_sorted::inverse() { 
	this->name="inversed";
	cout<<"Stack_sorted method used"<<endl;
	int n = this->length;
	if (n < 2)
	   return;
	              //если длина < 2 - ничего не делать
